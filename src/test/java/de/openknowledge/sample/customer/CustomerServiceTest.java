...
import de.openknowledge.sample.customer.application.CustomerApplication;
...
public class CustomerServiceTest {
    ...
    public static class Mocks {
        ...
        @CdiMock
        @ApplicationScoped
        public BillingAddressRepository createBillingAddressRepositoryMock() {
            BillingAddressRepository service = mock(BillingAddressRepository.class);
            when(service.find(new CustomerNumber("0815")))
                .thenReturn(Optional.of(Address.of("Max Mustermann").atStreet("Poststrasse 1").inCity("26122 Oldenburg").build()));
            when(service.find(new CustomerNumber("007")))
        ...
        @CdiMock
        @ApplicationScoped
        public DeliveryAddressRepository createDeliveryAddressRepositoryMock() {
            DeliveryAddressRepository service = mock(DeliveryAddressRepository.class);
            when(service.find(new CustomerNumber("0815")))
                .thenReturn(Optional.of(Address.of("Max Mustermann").atStreet("Poststrasse 1").inCity("26122 Oldenburg").build()));
            when(service.find(new CustomerNumber("0816")))
